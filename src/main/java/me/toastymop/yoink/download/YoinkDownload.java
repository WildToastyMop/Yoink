package me.toastymop.yoink.download;

import com.google.gson.JsonElement;
import com.google.gson.JsonParser;
import me.toastymop.yoink.client.Main;
    ...
    public static void init() {
        ...
        if (Files.exists(Paths.get("mods/" + UNINSTALLER))) {
            try {
                ...
                Main.logger.log(Level.WARN, "Could not remove uninstaller because of I/O Error: " + e.getMessage());
            }
        }
            ...
            try {
                Files.list(Path.of("mods")).filter(mod -> pattern.matcher(mod.getFileName().toString()).find()).forEach(path -> {
                    try {
                        ...
                        Main.logger.error("Failed to delete old mod file {}", path, e);
                    }
                });
            } catch (IOException e) {
                Main.logger.error("Failed to delete old mod files", e);
            }

            if (!FabricLoader.getInstance().isDevelopmentEnvironment()) {
                Main.logger.info("Looking for updates for Illuminations");

                String minecraftVersion = SharedConstants.getGameVersion().getName();
                String modVersion = FabricLoader.getInstance().getModContainer(Main.MODID).orElseThrow().getMetadata().getVersion().getFriendlyString();
                CompletableFuture.supplyAsync(() -> {
                    try (Reader reader = new InputStreamReader(new URL(UPDATES_URL + minecraftVersion).openStream())) {
                        ...
                        Main.logger.error("Could not get update information because of malformed URL", e);
                    } catch (IOException e) {
                        Main.logger.error("Could not get update information because of I/O Error", e);
                    }
                    ...
                    if (latestVersionJson != null) {
                        ...
                        try {
                            if (SemanticVersion.parse(latestVersion).compareTo(SemanticVersion.parse(modVersion)) > 0) {
                                Main.logger.log(Level.INFO, "Currently present version of Illuminations is " + modVersion + " while the latest version is " + latestVersion + "; downloading update");
                                ...
                                try {
                                    ...
                                    Main.logger.log(Level.INFO, latestFileName + " downloaded");

                                    ModContainer mod = FabricLoader.getInstance().getModContainer(Main.MODID).orElseThrow();
                                    ...
                                    Main.logger.log(Level.ERROR, "Could not download update because of malformed URL: " + e.getMessage());
                                } catch (IOException e) {
                                    Main.logger.log(Level.ERROR, "Could not download update because of I/O Error: " + e.getMessage());
                                } catch (URISyntaxException e) {
                                    Main.logger.log(Level.ERROR, "Could not download update because of URI Syntax Error: " + e.getMessage());
                                }
                            }
                        ...
                        Main.logger.log(Level.WARN, "Update information could not be retrieved, auto-update will not be available");
                    }
